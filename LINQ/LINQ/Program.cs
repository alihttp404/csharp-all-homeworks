...
var listFilterWithEmails = debtors.Where(db => db.Email.EndsWith("rhyta.com") || db.Email.EndsWith("dayrep.com")).ToList();
//foreach (var db in listFilterWithEmails) Console.WriteLine(db);

// 3 - Yashi 26-dan 36-ya qeder olan borclulari cixartmag
var listFilterWithAge = debtors.Where(db => db.BirthDay.Year - DateTime.Now.Year <= 36 && 26 <= db.BirthDay.Year - DateTime.Now.Year).ToList();
//foreach (var db in listFilterWithAge) Console.WriteLine(db);

// 4 - Borcu 5000-den cox olmayan borclularic cixartmag
var listFilterWithDebtAmount = debtors.Where(db => db.Debt > 5000).ToList();
//foreach (var db in listFilterWithDebtAmount) Console.WriteLine(db);

// 5 - Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclulari cixartmaq
var listFilterWithNameAndPhone = debtors.Where(db => db.Phone.Count(a => a == '7') >= 2 && db.FullName.Length > 18).ToList();
//foreach (var db in listFilterWithNameAndPhone) Console.WriteLine(db);

// 7 - Qishda anadan olan borclulari cixardmaq
var listFilterWithBornInWinter = debtors.Where(db => 1 == db.BirthDay.Month || db.BirthDay.Month == 2 || db.BirthDay.Month == 12).ToList();
//foreach (var db in listFilterWithBornInWinter) Console.WriteLine(db);
...
var listFilterWithDebtMoreThanAvg = debtors.Where(db => db.Debt > debtors.Average(db => db.Debt)).ToList();
var listFilterWithDebtMoreThanAvgAndSurname = listFilterWithDebtMoreThanAvg.OrderBy(db => db.FullName).ToList();
var listFilterWithDebtMoreThanAvgAndSurnameAndDebt = listFilterWithDebtMoreThanAvgAndSurname.OrderBy(db => db.Debt).ToList();
//foreach (var db in listFilterWithDebtMoreThanAvgAndSurnameAndDebt) Console.WriteLine(db);

// 9 - Telefon nomresinde 8 olmayan borclularin yalniz familyasin,yashin ve umumi borcun meblegin cixarmaq
var listFilterWithNoNum8 = debtors.Where(db => !db.Phone.Contains('8')).ToList();
//foreach (var db in listFilterWithNoNum8)
//{
//    Console.WriteLine(db.FullName);
//    Console.WriteLine(db.BirthDay);
//    Console.WriteLine(db.Debt);
//}
...
// chatgpt den komek almisam bunda
var ListFilterWith3SameLettersInName = debtors.Where(db =>
            db.FullName.GroupBy(c => c).Any(group => group.Count() >= 3)
        ).ToList();
var ListFilterWith3SameLettersInNameSortedInAlphabetical = ListFilterWith3SameLettersInName.OrderBy(db => db.FullName).ToList();
//foreach (var db in ListFilterWith3SameLettersInNameSortedInAlphabetical) Console.WriteLine(db);

// 13 - borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq
var MostFrequentBirthYear = debtors.MaxBy(db => db.BirthDay.Year);
//Console.WriteLine(MostFrequentBirthYear.BirthDay.Year);

// 14 - Borcu en boyuk olan 5 borclunun siyahisini cixartmaq
var Top5MostDebts = debtors.OrderBy(db => db.Debt).Take(5).ToList();
//foreach (var db in Top5MostDebts) Console.WriteLine(db);

// 15 - Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq
var AllDebts = debtors.Sum(db => db.Debt);
//Console.WriteLine(AllDebts);

// 16 - 2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
var HasSeenWW2 = debtors.Where(db => 1939 <= db.BirthDay.Year && db.BirthDay.Year <= 1945);
//foreach (var db in HasSeenWW2) Console.WriteLine(db);

// 18 - Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq
// yaza bilmedim

// 19 - Tesevvur edek ki,butun borclari olanlar
// bugunden etibaren her ay 500 azn pul odeyecekler. Oz ad gunune kimi borcun oduyub qurtara bilenlerin siyahisin cixartmaq
var CanPayDebtUntilBirthday = debtors.Where(db => (DateTime.Now.Month - db.BirthDay.Month) * 500 >= db.Debt);
//foreach (var db in CanPayDebtUntilBirthday) Console.WriteLine(db);

// 20 - Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq
var A = debtors.Where(db => db.FullName.ToLower().Contains('s')
                            && db.FullName.ToLower().Contains('m')
                            && db.FullName.ToLower().Contains('i')
                            && db.FullName.ToLower().Contains('l')
                            && db.FullName.ToLower().Contains('e'));
foreach (var db in A) Console.WriteLine(db);