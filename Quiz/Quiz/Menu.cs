...
namespace Quiz
{
    internal class Menu<T>
    {
        ...
        public Menu(ref List<T> options) => _options = options;

        public Menu(List<T> options) => _options = options;

        public Menu(ref List<T> options, ConsoleColor color)
        {
            _options = options;
            MenuColor = color;
        }

        public Menu(List<T> options, ConsoleColor color)
        {
            _options = options;
            MenuColor = color;
        }
        ...
        public void Print(ref int option)
        {
            ...
            for (int i = 0; i < _options.Count; i++)
            {
                if (option == i)
                {
                    ...
                    Console.WriteLine(String.Format("{0," + ((Console.WindowWidth / 2) + (_options[i].ToString().Length / 2)) + "}", _options[i]));
                    Console.ResetColor();
                }
                else Console.WriteLine(String.Format("{0," + ((Console.WindowWidth / 2) + (_options[i].ToString().Length / 2)) + "}", _options[i]));
            }
        }
